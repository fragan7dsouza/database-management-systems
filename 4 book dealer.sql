...
author A, catalogue C,order_det O
...
/* 2. Increase the price of the books published by a specific publisher
by 10% */
...
select count(ordno) as 'NO_OF_ORD',
bookid from ORDER_DET group by
bookid having sum(qty) <=all(select
sum(qty) from ORDER_DET group by bookid)